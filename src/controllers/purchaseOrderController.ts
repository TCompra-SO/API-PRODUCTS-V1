...
const getPurchaseOrderByUser = async (req: Request, res: Response) =>{
  ...
  try {
    const responseUser = await PurchaseOrderService.getPurchaseOrderByUser(userClientID);
    if (responseUser && responseUser.success) {
      res.status(responseUser.code).send(responseUser);
    } else {
      res.status(responseUser.code).send(responseUser.error);
...
export { CreatePurchaseOrderController, getPurchaseOrdersController, getPurchaseOrderByUser, getPurchaseOrderIDController };
